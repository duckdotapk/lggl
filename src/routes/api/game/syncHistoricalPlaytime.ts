...
import { callSteamPlayServiceClientGetLastPlayedTimesV1 } from "../../../libs/third-party/Steam.js";

import { ApiError, createEndpointRoute } from "../../../libs/Api.js";

import * as schema from "./syncHistoricalPlaytime.schemas.js";

//
// Locals
//

async function createHistoricalGamePlaySession
(
	game_id: number,
	platform_id: number,
...
)
{
	...
	const nonHistoricalPlayTimeSeconds = gamePlaySessions.reduce(
		(accumulator, gamePlaySession) => accumulator + gamePlaySession.playTimeSeconds,
		0
	);
...
export const route = createEndpointRoute<RouteFritterContext, typeof schema.RequestBodySchema, typeof schema.ResponseBodySchema>(
{
	...
	handler: async (requestBody) =>
	{
		...
		if (game == null)
		{
			throw new ApiError({ code: "NOT_FOUND", message: "Game not found." });
		}
		...
		switch (requestBody.provider.name)
		{
			case "steam":
			{
				...
				const clientLastPlayedTimes = await callSteamPlayServiceClientGetLastPlayedTimesV1(
					LGGL_STEAM_API_KEY,
				);

				const steamPlayedGame = clientLastPlayedTimes.games.find(
					(steamPlayedGame) => steamPlayedGame.appid == game.steamAppId,
				);
				...
				if (steamPlayedGame != null)
				{
						...
						await prismaClient.game.update(
						{
							...
							data:
							{
								firstPlayedDate: DateTime
									.fromSeconds(steamPlayedGame.first_playtime)
									.toJSDate(),
						...
						await prismaClient.game.update(
						{
							...
							data:
							{
								lastPlayedDate: DateTime
									.fromSeconds(steamPlayedGame.last_playtime)
									.toJSDate(),
							},
						});
					}
					...
					const linuxPlayTimeSeconds =
					(
						steamPlayedGame.playtime_linux_forever -
						steamPlayedGame.playtime_deck_forever
					) * 60;
					...
					await createHistoricalGamePlaySession(
						game.id,
						LGGL_PLATFORM_ID_WINDOWS,
						windowsPlayTimeSeconds,
					);
					await createHistoricalGamePlaySession(
						game.id,
						LGGL_PLATFORM_ID_MAC,
						macPlayTimeSeconds,
					);
					await createHistoricalGamePlaySession(
						game.id,
						LGGL_PLATFORM_ID_LINUX,
						linuxPlayTimeSeconds,
					);
					await createHistoricalGamePlaySession(
						game.id,
						LGGL_PLATFORM_ID_STEAM_DECK,
						steamDeckPlayTimeSeconds,
					);
					await createHistoricalGamePlaySession(
						game.id,
						LGGL_PLATFORM_ID_UNKNOWN,
		...
		const playTimeTotalSeconds = gamePlaySessions.reduce(
			(accumulator, gamePlaySession) => accumulator + gamePlaySession.playTimeSeconds,
			0,
		);
