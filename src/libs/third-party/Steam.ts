...
export async function fetchSteamAppDetails(steamAppId: number)
{
	...
	const response = await fetch(
		"https://store.steampowered.com/api/appdetails?" + searchParameters.toString(),
	);
...
export async function callSteamCommunityServiceGetAppsV1(steamAppIds: number[])
{
	const searchParameters = new URLSearchParams();
	...
	const response = await fetch(
		"https://api.steampowered.com/ICommunityService/GetApps/v1/?" + searchParameters.toString(),
	);

	const responseParseResult = ICommunityServiceGetAppsV1ResponseSchema.safeParse(
		await response.json(),
	);
...
export async function callSteamPlayServiceClientGetLastPlayedTimesV1(steamApiKey: string)
{
	const searchParameters = new URLSearchParams();
	...
	const responseParseResult = IPlayerServiceClientGetLastPlayedTimesV1ResponseSchema.safeParse(
		await response.json(),
	);
	...
	{
		throw new Error(
			"Failed to get client last played times: " +
			JSON.stringify(responseParseResult.error),
		);
	}
...
export async function callSteamPlayerServiceGetOwnedGamesV1
(
	steamApiKey: string,
	steamUserId: string,
)
...
export async function fetchSteamImageUrls(steamAppId: number)
{
	const communityServiceGetAppsResponse = await callSteamCommunityServiceGetAppsV1(
	[
		steamAppId,
	]);
	...
	return {
		icon: "https://cdn.cloudflare.steamstatic.com/steamcommunity/public/images/apps/" +
			steamAppId +
			"/" + app.icon + ".jpg",
		libraryBackground: "https://cdn.cloudflare.steamstatic.com/steam/apps/" +
			steamAppId +
			"/library_hero.jpg",
		libraryCapsule: "https://cdn.cloudflare.steamstatic.com/steam/apps/" +
			steamAppId +
			"/library_600x900_2x.jpg",
		libraryLogo: "https://cdn.cloudflare.steamstatic.com/steam/apps/" +
			steamAppId +
			"/logo.png",
	};
}